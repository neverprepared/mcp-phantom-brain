...
// Phase 6: TestSynthesizer_* removed with the file-queue synthesizer.
// SynthWorker (internal/server/synth_queue_test.go) covers the
// equivalent gate + distill + entity-extraction flow against OS.
...
// TestSlugify removed in Phase 6 with the synthesizer's file-write
// helpers — entity-doc IDs use osearch.EntitySlug now (covered in
// internal/osearch/docs_test.go).
