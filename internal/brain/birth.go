...
func Birth(opts BirthOpts) (brainDir string, m *Manifest, err error) {
	...
	// Phase 1: snapshot fetch is stubbed. Always greenfield. The seed
	// source field tells operators why their brain started blank.
	logger.Warn(
		"phantom-brain: daemon snapshot fetch stubbed in Phase 1 — birthing greenfield",
		slog.String("brain_id", brainID),
		slog.String("profile", opts.Agent.Profile),
		slog.String("vault", opts.Agent.Vault),
	)
